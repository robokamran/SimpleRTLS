...
#include "i2c.h"
#define i2c_write(slave_addr, reg_addr, length, pdata) \
        HAL_I2C_Mem_Write(&hi2c3, slave_addr << 1, reg_addr, 1, pdata, length, 200)
#define i2c_read(slave_addr, reg_addr, length, pdata)  \
        HAL_I2C_Mem_Read (&hi2c3, slave_addr << 1, reg_addr, 1, pdata, length, 200)
